...
for attributes such as `copilot_*`, `is_group_sharing_disabled`, `limit_sharing_mode`, `max_limit_user_sharing`, etc. This is a **provider-side issue** (reported for v4.1.0) that is **distinct from #931** — the crash was fixed in v4.0.0, but attribute-level invalid state for group-managed environments is a separate unresolved issue.
